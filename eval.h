...
#else
// FreeBSD / Darwin definition of PATH_MAX
...
int eval_error(const char *restrict, ...);
int eval_info(const char *restrict, ...);
int eval_success(const char *restrict, ...);
int eval_complete( char[] );

int datalog(const char *restrict , ...);
int successlog(const char *restrict , ...);
int errorlog(const char *restrict , ...);

int eval_check_successlog( const char *restrict , ...);
int eval_check_errorlog( const char *restrict , ...);
...
#define EVAL_CATCH( _code ) { \
    ...
    if ( !_eval_env.stat ) { \
        ...
        case(EVAL_CATCH_ABORT): \
            printf("\033[1;33m ⊣ \033[0m %s terminated by abort()\n", #_code ); \
            break; \
        case(EVAL_CATCH_BLOCKED): \
            printf("\033[1;33m ⊣ \033[0m %s terminated by calling blocked function\n", #_code ); \
            break; \
        ...
        case(EVAL_CATCH_LOG_OVERFLOW): \
            printf("\033[1;33m ⊣ \033[0m %s terminated due to log buffer overflow\n", #_code ); \
            break; \
        default: \
...
// Also override _exit() and _Exit()

#define _exit( status ) _eval_exit( status );
...
extern _eval_abort_type _eval_abort_data;

void _eval_abort(void);

#define abort( ) _eval_abort( );
